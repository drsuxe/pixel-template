          ...
          <template v-else>
            <mp-table-row v-for="(item, index) in datas" :key="`${item.id}`">
              ...
              <template v-if="$scopedSlots.default === undefined">
                <mp-table-cell v-for="(value, key, index) in item" :key="`${key}-${index}`" as="td" scope="row"> {{ value }} </mp-table-cell>
              </template>
            </mp-table-row>
          </template>
        </mp-table-body>
            ...
            <mp-popover-trigger>
              <mp-box>
                <mp-tooltip label="Result per page" position="bottom">
                  ...
                  >
                    <mp-text font-weight="600" line-height="md"> {{ currentPerPage }} </mp-text>
...
export default {
  ...
  mounted() {
    ...
    this.calculatePagination({ currentPerPage: this.perPage, totalData: this.totalData });
  },
  watch: {
  ...
  methods: {
    ...
    handleChangePerPage(perPage) {
      ...
      this.calculatePagination({ currentPerPage: perPage, totalData: this.totalData });

      this.$emit("changePage", { perPage: this.currentPerPage, currentPage: this.currentPage, totalPage: this.totalPage });
    },
