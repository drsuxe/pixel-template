    ...
    <mp-table :is-hoverable="false">
      <mp-table-body v-if="hasChildren">
          ...
          >
            <mp-flex align="center" justify="space-between">
              <mp-flex align="center">
                <mp-icon name="drag" mr="2" class="handle" />
                ...
                >
                  <mp-tooltip
                    :id="`has-children-tooltip-edit-${data.id}`"
                    label="Edit"
                  >
                    <mp-button-icon
                      name="edit"
                  ...
                  <mp-tooltip
                    :id="`has-children-tooltip-remove-${data.id}`"
                    label="Remove"
                  >
                    <mp-button-icon
                      name="close"
              ...
              <mp-flex>
                <mp-tooltip
                  :id="`has-children-tooltip-move-up-${data.id}`"
                  label="Move up"
                >
                  <mp-button-icon
                    name="chevrons-up"
                ...
                <mp-tooltip
                  :id="`has-children-tooltip-move-down-${data.id}`"
                  label="Move down"
                >
                  <mp-button-icon
                    name="chevrons-down"
        ...
        <template v-for="(children, childrenIndex) in data.children">
          <mp-table-row
            v-if="children.label"
            :id="`has-children-label-${data.id}-${childrenIndex}`"
            :key="`has-children-label-${data.id}-${childrenIndex}`"
          >
            <mp-table-cell
              as="td"
              scope="row"
            ...
            >
              <mp-flex align="center" pl="8">
                ...
                >
                  <mp-tooltip
                    :id="`has-children-tooltip-edit-${data.id}-${childrenIndex}`"
                    label="Edit"
                  >
                    <mp-button-icon
                      name="edit"
                      size="sm"
                      tabindex="-1"
                      @click="handleEditChildren({ data, children })"
                    />
                  </mp-tooltip>

                  <mp-tooltip
                    :id="`has-children-tooltip-remove-${data.id}-${childrenIndex}`"
                    label="Remove"
                  >
                    <mp-button-icon
                      name="close"
                      size="sm"
                      tabindex="-1"
                      @click="handleRemoveChildren({ data, children })"
                    />
                  </mp-tooltip>
                </mp-box>
              </mp-flex>
          ...
          >
            <mp-table-cell as="td" scope="row" colspan="4">
              ...
              >
                ...
                >
                  <mp-tooltip
                    :id="`tooltip-remove-${data.id}-${childrenIndex}-${account.id}`"
                    label="Remove"
                  >
                    <mp-button-icon
                      name="close"
                      tabindex="-1"
                      @click="handleRemoveAccount({ data, children, account })"
                    />
                  </mp-tooltip>
                </mp-box>
              </mp-flex>
          ...
          >
            <mp-flex align="center" justify="space-between">
              <mp-flex align="center">
                ...
                >
                  <mp-tooltip
                    :id="`no-children-tooltip-edit-${data.id}`"
                    label="Edit"
                  >
                    <mp-button-icon
                      tabindex="-1"
                  ...
                  <mp-tooltip
                    :id="`no-children-tooltip-remove-${data.id}`"
                    label="Remove"
                  >
                    <mp-button-icon
                      tabindex="-1"
              ...
              <mp-flex>
                <mp-tooltip
                  :id="`no-children-tooltip-move-up-${data.id}`"
                  label="Move up"
                >
                  <mp-button-icon
                    name="chevrons-up"
                ...
                <mp-tooltip
                  :id="`no-children-tooltip-move-down-${data.id}`"
                  label="Move down"
                >
                  <mp-button-icon
                    name="chevrons-down"
