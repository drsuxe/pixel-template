<template>
  <mp-flex position="relative" justify-content="center" align-items="center" flex-direction="row" flex="1 0 0%">
    <mp-flex
      flex="1 0 0%"
      min-height="auto"
      border-top-width="md"
    ...
    <mp-pseudo-box role="group" :_hover="{ cursor: 'pointer' }" @click="$emit('click', id)">
      <mp-flex
        width="7"
        height="7"
        ...
        :outline-color="getStepProps({ isCompleted: 'blue.400', isCurrent: 'blue.400' }, 'gray.100')"
        :_groupHover="{
          outlineColor: getStepProps({ isCompleted: 'blue.500' }, 'gray.100'),
          backgroundColor: getStepProps({ isCompleted: 'blue.500', isCurrent: 'blue.50' }, 'gray.100'),
        }"
      >
        <mp-icon v-if="isCompleted" name="check" size="sm" color="white" />
        <mp-text v-else font-size="md" font-weight="semibold" text-align="center" :color="getStepProps({ isCurrent: 'blue.400' }, 'gray.600')">
          {{ id }}
        </mp-text>
      </mp-flex>
      <mp-flex justify-content="center" align-items="center" flex-direction="column" text-align="center">
        <mp-text
          font-size="md"
          text-align="center"
          margin-top="1"
